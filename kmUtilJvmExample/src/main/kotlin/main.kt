		...
		args.asList().iterator().apply args@{
			for (arg in this) {
				when (arg) {
					...
					"--test"     -> isTest = true
					"--debug"    -> debugLevel++
					"--dir"      -> dir = if (hasNext()) next() else ""
					"-", ""      -> names += arg
					"--"         -> names.addAll(asSequence()) // append remaining elements
					else         -> {
						if (arg.isNotEmpty() && arg[0] == '-') {
							// handle short args here
							arg.slice(1..arg.lastIndex).iterator().apply arg@{
								for (ch in this) {
									when (ch) {
										'-'      -> usage("unknown long param \"$arg\"")
										'h', 'u' -> usage()
										'd'      -> ++debugLevel
										's'      -> isSimulate = true
										'q'      -> {
										...
										else     -> usage("unknown short  param \"-$ch\"")
									}
								}
							}
						} else {
...
fun main(args: Array<String>) {
	...
	logMessage('I', "kmutil-jvm example start (${System.getProperty("app.name")} on ${System.getProperty("os.name")})")
